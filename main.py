...
async def prof_user(message: types.Message):
    ...
    ''', parse_mode='html')
        ...
        if message.text.lower() in ['!беседа', '!Беседа']:
            ...
            await bot.send_message(message.chat.id, f'{name} 💬Официальная беседа бота - @warkraftGame_chat', parse_mode='html')
        #######################################РП Команды#########################################
...
🔐 Деньги на депозите: 0$
    ...
    Наш telegram канал @slivmens''', parse_mode='html')
        #####################################################МАШИНЫ#############################################################
...
''', parse_mode='html')
       ...
       ###########################################АДМИН############################################
        ...
        if message.text.lower() in ["помощь", "Помощь"]:
            ...
            name1 = message.from_user.get_mention(as_html=True)
            await bot.send_message(message.chat.id,
                                   f'{name1}, выберите категорию:\n   1️⃣ Основное\n   2️⃣ Игры\n   3️⃣ Развлекательное\n   4️⃣ Кланы(В разработке...)\n   5️⃣Модерация чата\n   6️⃣Донат\n\n💬 Так же у нас есть <a href="https://off-bot.ru">свой форум</a>\n🆘 По всем вопросам - https://off-bot.ru',                  
                                   reply_markup=help, parse_mode='html')
...
async def inlinebutton(callback: types.CallbackQuery):
   ...
   🆙Список доступного доната
         ...
         3️⃣Префиксы
         4️⃣Скины
   🔹Для покупки доната обращайтесь к создателю бота - @VDVadimFTG 
