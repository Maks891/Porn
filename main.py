...
async def prof_user(message: types.Message):
    ...
    ''', parse_mode='html')
        ...
        if message.text.lower() in ['!беседа', '!Беседа']:
            ...
            await bot.send_message(message.chat.id, f'{name} 💬Официальная беседа бота - https://t.me/+ZMufFpTfSTo2ZmYy ', parse_mode='html')
        #######################################РП Команды#########################################
...
async def inlinebutton(callback: types.CallbackQuery):
   ...
   🆙Список доступного доната
         ...
         3️⃣Префиксы
         4️⃣Скины
   🔹Для покупки доната обращайтесь к создателю бота - @Geormor
